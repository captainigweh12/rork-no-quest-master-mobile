...
// --- CORS (include custom headers you use from the app) ---
app.use(
  "*",
  cors({
    origin: "*",
    ...
    allowHeaders: [
      ...
      // add any custom headers used by the app/backend:
      "X-AGORA-MINT-KEY",
...
// --- tRPC mount (IMPORTANT: set endpoint so routing strips /api/trpc correctly) ---
app.use(
  "/api/trpc/*",
  trpcServer({
    router: appRouter,
...
app.post("/api/auth/hook", async (c) => {
  ...
  try {
    ...
    if (webhookSecret && signature) {
      ...
      // NOTE: Add verification if you enable signed webhooks.
    } else {
...
async function handleEmailVerification(c: any, payload: any) {
  ...
  try {
    ...
    console.log("   ✅ Verification email sent! Message ID:", data?.id);
          ...
          <body>
            <div class="container">
              ...
              <div class="content">
                ...
                <p style="color: #666; font-size: 14px; margin-top: 30px;">Or copy and paste this link:<br><a href="${resetUrl}" style="color: #FF6B2C;">${resetUrl}</a></p>
                <p style="color: #999; font-size: 12px; margin-top: 30px;">⏱️ This link expires in 1 hour<br>If you didn't request this, you can safely ignore this email.</p>
              </div>
